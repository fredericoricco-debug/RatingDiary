...
#Fonts
INST_FONT = 'Arial 20' #Instruction font.
...
left_col = [[sg.Text("Welcome to %s" % (app_name)),
            sg.Text("", key = '-DONE-', size = (150,1))],[sg.Input(key =
            "-IN-", size = (150, 2)), sg.Submit(key = "-SUB-")],[sg.Button("Clear Input", key =
            "-CLR-"),sg.Button("Submit Diary", key = "-SUB2-"),
            sg.Button("Delete Diary", key = "-DEL-")],[sg.Multiline(size = (180, 60), key =
            "-FULL-", default_text = linebyline)]]

#Create Second Column
right_col = [[sg.Listbox(list(dict.keys()), enable_events = True, size = (30, 60),
